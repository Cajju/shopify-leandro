...
let { pricing, defaultTrialDays, BillingInterval, currencyCode } = shopifyConfig
  ...
  if (isRecurring(interval)) {
    ...
    do {
      ...
      for (let i = 0, len = purchases.edges.length; i < len; i++) {
        ...
        if (node.name === chargeName && (sharedConfig.env !== 'dev' || !node.test) && node.status === 'ACTIVE') {
          return true
        }
      }
...
async function requestRecurringPayment(client, returnUrl, { chargeName, amount, currencyCode, interval, trialDays }) {
  const mutationResponse = await client.request(RECURRING_PURCHASE_MUTATION, {
    variables: {
      ...
      returnUrl,
      test: sharedConfig.env !== 'dev' || client.domain === 'lead-hive.myshopify.com',
      trialDays
    }
  })
